...
function createProjectContext(opts) {
  ...
  var onGetGitBranches = opts.onGetGitBranches || null;
  ...
  function handleMessage(ws, msg) {
    ...
    if (msg.type === "create_worktree") {
      if (typeof onCreateWorktree === "function") {
        var wtResult = onCreateWorktree(msg.parentSlug, msg.branchName, msg.baseBranch);
    ...
    // --- List git branches for worktree dialog ---
    if (msg.type === "get_git_branches") {
      if (typeof onGetGitBranches === "function") {
        var branchResult = onGetGitBranches(msg.parentSlug);
        sendTo(ws, { type: "get_git_branches_result", ok: branchResult.ok, branches: branchResult.branches, error: branchResult.error });
      } else {
        sendTo(ws, { type: "get_git_branches_result", ok: false, error: "Not supported" });
      }
      return;
    }
