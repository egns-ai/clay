...
function createProjectContext(opts) {
  ...
  var parentSlug = opts.parentSlug || null;
  ...
  function computeAgentStatus(session) {
    if (!session) return "idle";
    if (!session.isProcessing) return "idle";
    var hasPendingPerms = session.pendingPermissions && Object.keys(session.pendingPermissions).length > 0;
    if (hasPendingPerms) return "waiting_permission";
    var hasPendingAsk = session.pendingAskUser && Object.keys(session.pendingAskUser).length > 0;
    if (hasPendingAsk) return "waiting_input";
    var hasPendingElicit = session.pendingElicitations && Object.keys(session.pendingElicitations).length > 0;
    if (hasPendingElicit) return "waiting_input";
    return "working";
  }

  function broadcastAgentStatus(session) {
    if (!session) return;
    var status = computeAgentStatus(session);
    for (var ws of clients) {
      var wsUser = ws._clayUser || null;
      if (usersModule.isMultiUser() && wsUser) {
        if (!usersModule.canAccessSession(wsUser.id, session, { visibility: "public" })) continue;
      } else if (!usersModule.isMultiUser() && session.ownerId) {
        continue;
      }
      sendTo(ws, { type: "agent_status", sessionId: session.localId, status: status });
    }
  }
  ...
  var sdk = createSDKBridge({
    ...
    onAgentStatusChange: broadcastAgentStatus,
  });
  ...
  function runJudge() {
    ...
    onProcessingChanged();
    broadcastAgentStatus(judgeSession);
  ...
  function handleConnection(ws, wsUser) {
    ...
    if (active) {
      ...
      var agentStatus = computeAgentStatus(active);
      if (agentStatus !== "idle") {
        sendTo(ws, { type: "agent_status", sessionId: active.localId, status: agentStatus });
      }
    }
  ...
  function handleMessage(ws, msg) {
    ...
    if (msg.type === "permission_response") {
      ...
      if (decision === "allow_clear_context") {
        ...
        Promise.race([
          ...
          try {
            ...
            onProcessingChanged();
            broadcastAgentStatus(newSession);
      ...
      // --- Plan approval: "allow_worktree" — create worktree + plan as first message + acceptEdits ---
      if (decision === "allow_worktree") {
        // Build plan prompt from plan content (sent from client) or plan file path
        var clientPlanContent = msg.planContent || "";
        var planPrompt;
        if (clientPlanContent) {
          planPrompt = "Execute the following plan. Do NOT re-enter plan mode — just implement it step by step.\n\n" + clientPlanContent;
        ...
        // 1. Deny current plan to end the turn
        pending.resolve({ behavior: "deny", message: "User chose to execute in worktree" });
        sm.sendAndRecord(session, { type: "permission_resolved", requestId: requestId, decision: decision });

        // 2. Abort old session (defer to next tick so deny write completes)
        session.isProcessing = false;
        onProcessingChanged();
        session.pendingPermissions = {};
        session.pendingAskUser = {};
        ...
        // 3. Derive branch name from plan content
        var branchName = "plan-" + Date.now();
        var nameMatch = clientPlanContent.match(/^#\s+(.+)/m) || clientPlanContent.match(/^(.{3,40})/);
        if (nameMatch) {
          var derived = nameMatch[1].toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "").slice(0, 40);
          if (derived) branchName = derived;
        }

        // 4. Create worktree
        if (typeof onCreateWorktree !== "function") {
          sendTo(ws, { type: "error", text: "Worktree creation not supported" });
          return;
        }
        // If the current project is itself a worktree (slug contains "--"), use the parent slug
        var parentSlugForWt = slug.indexOf("--") !== -1 ? slug.split("--")[0] : slug;
        var wtResult = onCreateWorktree(parentSlugForWt, branchName, "HEAD");
        if (!wtResult.ok) {
          sendTo(ws, { type: "error", text: "Failed to create worktree: " + (wtResult.error || "unknown") });
          return;
        }

        // 5. Tell client to switch to worktree and send plan
        sendTo(ws, {
          type: "worktree_plan_launch",
          slug: wtResult.slug,
          planPrompt: planPrompt,
          planContent: clientPlanContent,
        });
        return;
      }
    ...
    if (msg.type === "create_worktree") {
      if (typeof onCreateWorktree === "function") {
        var wtResult = onCreateWorktree(msg.parentSlug, msg.branchName, msg.baseBranch, msg.dirName);
    ...
    if (msg.type === "ralph_wizard_complete") {
      ...
      onProcessingChanged();
      broadcastAgentStatus(craftingSession);
