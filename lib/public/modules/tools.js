...
// --- Factory ---
function createToolsInstance(_ctx) {
  var ctx = _ctx;
  ...
  function markAskUserAnswered(toolId) {
  ...
  function renderPlanPermission(requestId) {
    ...
    // Option 1.5: Worktree & auto-accept
    var worktreeBtn = document.createElement("button");
    worktreeBtn.className = "permission-btn plan-btn-worktree";
    worktreeBtn.innerHTML = iconHtml("git-branch") + ' <span>Worktree &amp; auto-accept</span>';
    worktreeBtn.addEventListener("click", function () {
      sendPlanResponse(container, requestId, "allow_worktree");
    });
    ...
    actions.appendChild(worktreeBtn);
  ...
  function sendPlanResponse(container, requestId, decision, feedback) {
    ...
    var labelMap = {
      ...
      "allow_worktree": "Approved (worktree + auto-accept)",
      "deny": "Rejected",
    ...
    if (ctx.ws && ctx.connected) {
      ...
      if ((decision === "allow_clear_context" || decision === "allow_worktree") && planContent) {
  ...
  function markElicitationResolved(requestId, action) {
  ...
  function updateTodoStickyVisibility() {
    var stickyEl = document.querySelector(".pane-todo-sticky");
    if (!stickyEl) return;
  ...
  function stopThinking(duration) {
    if (!currentThinking) return;
      ...
      (function (toolName, toolInput) {
        descEl.onclick = function (e) {
          ...
          if (toolName === "Edit" && toolInput && (toolInput.old_string || toolInput.new_string)) {
            ctx.openFile(filePath, { diff: { oldStr: toolInput.old_string || "", newStr: toolInput.new_string || "" } });
          } else {
    ...
    if (filePath) {
      (function (fp, os, ns) {
        pathSpan.addEventListener("click", function (e) {
          e.stopPropagation();
          ctx.openFile(fp, { diff: { oldStr: os || "", newStr: ns || "" } });
        });
  ...
  function updateToolResult(id, content, isError, images) {
    var tool = tools[id];
    if (!tool) return;
  ...
  function updateSubagentActivity(parentToolId, text) {
  ...
  function addSubagentToolEntry(parentToolId, toolName, toolId, text) {
    var tool = tools[parentToolId];
    if (!tool || !tool.el) return;
  ...
  function updateSubagentProgress(parentToolId, usage, lastToolName, summary) {
    var tool = tools[parentToolId];
    if (!tool || !tool.el) return;
  ...
  function initSubagentStop(parentToolId, taskId) {
    var tool = tools[parentToolId];
    if (!tool || !tool.el) return;
  ...
  function markSubagentDone(parentToolId, status, summary, usage) {
    var tool = tools[parentToolId];
    if (!tool || !tool.el) return;
  ...
  function removeToolFromGroup(toolId) {
  ...
  function isInPlanMode() { return inPlanMode; }
  function getPlanContent() { return planContent; }
  function setPlanContent(c) { planContent = c; }
  function isPlanFilePath(fp) { return isPlanFile(fp); }
  function getPlanModeTools() { return PLAN_MODE_TOOLS; }
  function getTodoTools() { return TODO_TOOLS; }
  function getHiddenResultTools() { return HIDDEN_RESULT_TOOLS; }
  ...
  function restoreToolState(saved) {
    tools = saved.tools;
  ...
  return {
    ...
    disableMainInput: disableMainInput,
    ...
    getPlanModeTools: getPlanModeTools,
    getTodoTools: getTodoTools,
    getHiddenResultTools: getHiddenResultTools,
...
export function initTools(_ctx) { _primary = createToolsInstance(_ctx); }
export function toolSummary(name, input) { return _primary.toolSummary(name, input); }
export function toolActivityText(name, input) { return _primary.toolActivityText(name, input); }
export function renderAskUserQuestion(toolId, input) { return _primary.renderAskUserQuestion(toolId, input); }
export function disableMainInput() { return _primary.disableMainInput(); }
export function enableMainInput() { return _primary.enableMainInput(); }
export function markAskUserAnswered(toolId) { return _primary.markAskUserAnswered(toolId); }
export function renderPermissionRequest(requestId, toolName, toolInput, decisionReason) { return _primary.renderPermissionRequest(requestId, toolName, toolInput, decisionReason); }
export function markPermissionResolved(requestId, decision) { return _primary.markPermissionResolved(requestId, decision); }
export function markPermissionCancelled(requestId) { return _primary.markPermissionCancelled(requestId); }
export function renderElicitationRequest(msg) { return _primary.renderElicitationRequest(msg); }
export function markElicitationResolved(requestId, action) { return _primary.markElicitationResolved(requestId, action); }
export function renderPlanBanner(type) { return _primary.renderPlanBanner(type); }
export function renderPlanCard(content) { return _primary.renderPlanCard(content); }
export function handleTodoWrite(input) { return _primary.handleTodoWrite(input); }
export function handleTaskCreate(input) { return _primary.handleTaskCreate(input); }
export function handleTaskUpdate(input) { return _primary.handleTaskUpdate(input); }
export function startThinking() { return _primary.startThinking(); }
export function appendThinking(text) { return _primary.appendThinking(text); }
export function stopThinking(duration) { return _primary.stopThinking(duration); }
export function resetThinkingGroup() { return _primary.resetThinkingGroup(); }
export function createToolItem(id, name) { return _primary.createToolItem(id, name); }
export function updateToolExecuting(id, name, input) { return _primary.updateToolExecuting(id, name, input); }
export function updateToolResult(id, content, isError, images) { return _primary.updateToolResult(id, content, isError, images); }
export function markToolDone(id, isError) { return _primary.markToolDone(id, isError); }
export function markAllToolsDone() { return _primary.markAllToolsDone(); }
export function updateSubagentActivity(parentToolId, text) { return _primary.updateSubagentActivity(parentToolId, text); }
export function addSubagentToolEntry(parentToolId, toolName, toolId, text) { return _primary.addSubagentToolEntry(parentToolId, toolName, toolId, text); }
export function updateSubagentProgress(parentToolId, usage, lastToolName, summary) { return _primary.updateSubagentProgress(parentToolId, usage, lastToolName, summary); }
export function initSubagentStop(parentToolId, taskId) { return _primary.initSubagentStop(parentToolId, taskId); }
export function markSubagentDone(parentToolId, status, summary, usage) { return _primary.markSubagentDone(parentToolId, status, summary, usage); }
export function addTurnMeta(cost, duration) { return _primary.addTurnMeta(cost, duration); }
export function removeToolFromGroup(toolId) { return _primary.removeToolFromGroup(toolId); }
export function closeToolGroup() { return _primary.closeToolGroup(); }
export function getTools() { return _primary.getTools(); }
export function isInPlanMode() { return _primary.isInPlanMode(); }
export function getPlanContent() { return _primary.getPlanContent(); }
export function setPlanContent(c) { return _primary.setPlanContent(c); }
export function isPlanFilePath(fp) { return _primary.isPlanFilePath(fp); }
export function getPlanModeTools() { return _primary.getPlanModeTools(); }
export function getTodoTools() { return _primary.getTodoTools(); }
export function getHiddenResultTools() { return _primary.getHiddenResultTools(); }
export function saveToolState() { return _primary.saveToolState(); }
export function restoreToolState(saved) { return _primary.restoreToolState(saved); }
export function resetToolState() { return _primary.resetToolState(); }

export { createToolsInstance };
