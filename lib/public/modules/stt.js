...
export function initSTT(_ctx) {
  ...
  sttBtn = ctx.inputEl.closest('.pane').querySelector('.pane-stt-btn');
  if (!sttBtn) return;
...
function showLangPopover() {
  ...
  var wrapper = ctx.inputEl.closest('.pane').querySelector('.pane-input-wrapper');
  if (wrapper) wrapper.appendChild(langPopover);
