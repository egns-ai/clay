...
export function createTerminalInstance(_ctx) {
  var ctx = _ctx;
  ...
  // Close panel button
  var termCloseBtn = ctx.paneEl.querySelector(".pane-terminal-close");
  if (termCloseBtn) termCloseBtn.addEventListener("click", function () {
    closeTerminal();
  });

  // Fullscreen toggle
  var termFullscreenBtn = ctx.paneEl.querySelector(".pane-terminal-fullscreen");
  if (termFullscreenBtn) termFullscreenBtn.addEventListener("click", function () {
  ...
  var toggleBtn = ctx.paneEl.querySelector(".pane-terminal-toggle-btn");
  if (toggleBtn) {
  ...
  // Sidebar terminal button (global singleton)
  var sidebarTermBtn = document.getElementById("terminal-sidebar-btn");
  if (sidebarTermBtn) {
    sidebarTermBtn.addEventListener("click", function () {
      if (ctx.closeSidebar) ctx.closeSidebar();
      openTerminal();
    });
  }

  // New tab button
  var newTabBtn = ctx.paneEl.querySelector(".pane-terminal-new-tab");
  if (newTabBtn) {
  ...
  function openTerminal() {
    ...
    if (ctx.closeFileViewer) ctx.closeFileViewer();
  ...
  function closeTerminal() {
    ...
    var fsIcon = ctx.paneEl.querySelector(".pane-terminal-fullscreen [data-lucide]");
    if (fsIcon) {
    ...
    // Hide toolbar
    var toolbar = ctx.paneEl.querySelector(".pane-terminal-toolbar");
    if (toolbar) {
  ...
  function renderTabBar() {
    var tabsEl = ctx.paneEl.querySelector(".pane-terminal-tabs");
    if (!tabsEl) return;
  ...
  function updateTerminalBadge() {
    var countEl = ctx.paneEl.querySelector(".pane-terminal-count");
  ...
  function handleTermCreated(msg) {
  ...
  function setTerminalTheme(xtermTheme) {
    for (var tab of tabs.values()) {
      if (tab.xterm) {
      ...
      tab.xterm.attachCustomKeyEventHandler(function (ev) {
        if (ctrlActive && ev.type === "keydown" && ev.key.length === 1) {
          ...
          if (charCode >= 65 && charCode <= 90) {
            ...
            ctrlActive = false;
            var ctrlBtn = ctx.paneEl.querySelector(".pane-terminal-toolbar [data-key='ctrl']");
            if (ctrlBtn) ctrlBtn.classList.remove("active");
            return false;
          }
        }
...
var _primary = null;
export function initTerminal(_ctx) { _primary = createTerminalInstance(_ctx); }
export function openTerminal() { return _primary.openTerminal(); }
export function closeTerminal() { return _primary.closeTerminal(); }
export function handleTermList(msg) { return _primary.handleTermList(msg); }
export function handleTermCreated(msg) { return _primary.handleTermCreated(msg); }
export function handleTermOutput(msg) { return _primary.handleTermOutput(msg); }
export function handleTermExited(msg) { return _primary.handleTermExited(msg); }
export function handleTermClosed(msg) { return _primary.handleTermClosed(msg); }
export function resetTerminals() { return _primary.resetTerminals(); }
export function sendTerminalCommand(command) { return _primary.sendTerminalCommand(command); }
export function setTerminalTheme(xtermTheme) { return _primary.setTerminalTheme(xtermTheme); }
