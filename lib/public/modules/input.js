...
// --- Factory ---
export function createInputInstance(_ctx) {
  var ctx = _ctx;
  ...
  function sendMessage() {
    ...
    if (text === "/rewind") {
      ...
      if (ctx.messageUuidMap().length === 0) {
        ...
        ctx.setRewindMode(!ctx.isRewindMode());
      }
      return;
    }
  ...
  var attachFileBtn = ctx.paneEl.querySelector(".pane-attach-file-btn");
  if (attachFileBtn) {
  ...
  var attachImageBtn = ctx.paneEl.querySelector(".pane-attach-image-btn");
  if (attachImageBtn) {
  ...
  // Paste handler — scoped to this pane's input
  ctx.inputEl.addEventListener("paste", function (e) {
...
export function initInput(_ctx) { _primary = createInputInstance(_ctx); }
export function sendMessage() { return _primary.sendMessage(); }
export function autoResize() { return _primary.autoResize(); }
export function clearPendingImages() { return _primary.clearPendingImages(); }
export function hideSlashMenu() { return _primary.hideSlashMenu(); }
export function handleInputSync(text) { return _primary.handleInputSync(text); }
