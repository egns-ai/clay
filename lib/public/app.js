...
// --- DOM refs ---
  ...
  // --- Worktree plan launch ---
  var pendingWorktreePlan = null;
  ...
  function connect() {
    ...
    ws.onopen = function () {
      ...
      // Execute pending worktree plan (from "Worktree & auto-accept" button)
      if (pendingWorktreePlan) {
        var plan = pendingWorktreePlan;
        pendingWorktreePlan = null;
        ws.send(JSON.stringify({ type: "set_permission_mode", mode: "acceptEdits" }));
        setTimeout(function () {
          if (ws && ws.readyState === 1) {
            ws.send(JSON.stringify({ type: "message", text: plan.planPrompt }));
          }
        }, 300);
      }
    };
  ...
  function processMessage(msg) {
      switch (msg.type) {
        ...
        case "worktree_plan_launch":
          pendingWorktreePlan = { planPrompt: msg.planPrompt, planContent: msg.planContent };
