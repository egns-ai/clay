...
// --- DOM refs ---
  ...
  function renderHomeHub(projects) {
    ...
    // --- Certificate download (TLS only) ---
    var certCard = $("hub-cert-download");
    if (certCard) {
      if (location.protocol === "https:") {
        certCard.classList.remove("hidden");
        var copyBtn = $("hub-cert-copy");
        if (copyBtn && !copyBtn._hubWired) {
          copyBtn._hubWired = true;
          copyBtn.addEventListener("click", function () {
            var code = $("hub-cert-code");
            if (code) {
