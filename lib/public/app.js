...
// --- DOM refs ---
  ...
  function updateProjectList(msg) {
    ...
    if (msg.allUsers) {
      ...
      renderUserStrip(msg.allUsers, onlineIds, myUserId, cachedDmFavorites, cachedDmConversations, dmUnread);
  ...
  function processMessage(msg) {
      switch (msg.type) {
        ...
        case "dm_message":
          if (dmMode && msg.dmKey === dmKey) {
            ...
            if (fromId && fromId !== myUserId) {
              ...
              renderUserStrip(cachedAllUsers, onlineIdsForDm, myUserId, cachedDmFavorites, cachedDmConversations, dmUnread);
        ...
        case "dm_favorites_updated":
          ...
          renderUserStrip(cachedAllUsers, onlineIds2, myUserId, cachedDmFavorites, cachedDmConversations, dmUnread);
          break;
  ...
  function handleRemoveProjectCheckResult(msg) {
    ...
    if (msg.count > 0) {
      ...
      showConfirm('Remove "' + name + '" from Clay? Your files will not be deleted.', function () {
        if (ws && ws.readyState === 1) {
